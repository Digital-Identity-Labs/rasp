...
RUN install_packages curl runit apache2 openssl ca-certificates krb5-user \
    libapache2-mod-shib libapache2-mod-auth-gssapi libapache2-mod-auth-openidc libapache2-mod-auth-cas libapache2-mod-security2 && \
...
RUN chmod a+x /etc/service/**/run && touch /etc/inittab && \
    ...
    a2dismod cgi cgid auth_gssapi auth_openidc auth_cas security2 && \
